...
export function OwnerRoomClient({ initialEntries, username, avatar: _avatar = 'girl', initialAdd = false }: { initialEntries: MediaEntry[]; username: string; avatar?: 'girl' | 'boy'; initialAdd?: boolean }) {
  ...
  return <>
    ...
    <div className="handoff-room-shell"><HandoffFrame src="/handoff/landing.dc.html" title={`${username}'s room`} shelves={toHandoffShelves(entries)} />{shelfOverflow > 0 && <button className="handoff-overflow" type="button" onClick={() => setManageZone('reading-shelf')}>+{shelfOverflow} more</button>}</div>
