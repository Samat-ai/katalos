...
import { MediaEditorOverlay } from '@/components/room/MediaEditorOverlay';
...
export function OwnerRoomClient({ initialEntries, displayName, username, avatar = 'girl', initialAdd = false }: { initialEntries: MediaEntry[]; displayName?: string; username: string; avatar?: 'girl' | 'boy'; initialAdd?: boolean }) {
  ...
  return <>
    <RoomActionBar displayName={displayName || username} username={username} onAdd={() => setEditing(null)} onCopy={() => void copyPublicLink()} />
    {message && <p role="status">{message}</p>}
    <MediaRoom entries={entries} avatar={avatar} onEdit={setEditing} onDelete={(entry) => setPendingDelete(entry.id)} onZoneOverflow={setManageZone} />
    {editing !== undefined && <MediaEditorOverlay title={editing ? 'EDIT MEDIA' : 'ADD MEDIA'} onClose={() => setEditing(undefined)}><MediaEntryForm initialEntry={editing ?? undefined} onSave={save} onCancel={() => setEditing(undefined)} /></MediaEditorOverlay>}
