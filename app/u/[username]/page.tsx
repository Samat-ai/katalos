...
export default async function PublicProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  return <main className="owner-page"><header><p className="eyebrow">@{profile.username}</p><h1>{profile.display_name}&apos;s room</h1></header>{entries.length ? <MediaRoom entries={entries} readOnly /> : <p>This room has no shared media yet.</p>}</main>;
}
