...
export function MediaEntryForm({ initialEntry, onSave, onCancel }: { initialEntry?: MediaEntry; onSave: (entry: MediaEntryInput) => void | Promise<void>; onCancel?: () => void }) {
  ...
  return <form className="entry-form media-workbench" onSubmit={submit} noValidate>
    <section className="form-step"><h3>1 · TYPE</h3><div className="type-tiles">{(['book', 'manga', 'anime', 'movie'] as const).map((type) => <button key={type} type="button" aria-pressed={values.type === type} onClick={() => update('type', type)}><span className={`type-tile-mark ${type}`} aria-hidden="true" />{type}</button>)}</div></section>
