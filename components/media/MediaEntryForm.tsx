...
const EntrySchema = z.object({
  ...
  coverUrl: z.string().trim(), synopsis: z.string().trim(), rating: z.string(), note: z.string().trim(), visibility: z.enum(['public', 'private']),
});
export type MediaEntryInput = Omit<MediaEntry, 'id'>;
...
export function MediaEntryForm({ initialEntry, onSave, onCancel }: { initialEntry?: MediaEntry; onSave: (entry: MediaEntryInput) => void; onCancel?: () => void }) {
  ...
  const submit = (event: React.FormEvent) => { event.preventDefault(); const parsed = EntrySchema.safeParse(values); if (!parsed.success) { setError(parsed.error.issues[0]?.message ?? 'Please check this entry'); return; } setError(''); onSave({ ...parsed.data, coverUrl: parsed.data.coverUrl || undefined, rating: parsed.data.rating ? Number(parsed.data.rating) : undefined, note: parsed.data.note || undefined }); };
