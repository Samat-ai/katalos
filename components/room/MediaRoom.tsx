...
export function MediaRoom({ entries, readOnly: _readOnly, avatar = 'girl' }: { entries: MediaEntry[]; readOnly: boolean; avatar?: 'girl' | 'boy' }) {
  ...
  const renderNook = (title: string, items: MediaEntry[], zoneNames: RoomZone[], nook: string) => <NookScaleWrapper><section className={`room-nook knook ${nook} avatar-${avatar}`} aria-label={title}><div className="nook-plaque">{title}</div><div className="nook-window" aria-hidden="true"><span /><span /></div><div className="pixel-sprite" aria-hidden="true"><i /><b /></div><div className="nook-furniture" aria-hidden="true" /><div className="nook-scene">{zoneNames.map((zone) => <div className="scene-zone" key={zone}>{renderZone(zone, items.filter((entry) => getRoomZone(entry) === zone))}</div>)}</div></section></NookScaleWrapper>;
