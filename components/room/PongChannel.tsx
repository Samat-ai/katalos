...
export function PongChannel({ active }: { active: boolean }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let y = 2; y < height; y += 8) context.fillRect(width / 2 - 1, y, 2, 4);
      context.fillStyle = '#7fe36b'; context.fillRect(2, Math.round(state.playerY), paddleWidth, paddleHeight); context.fillRect(width - paddleWidth - 2, Math.round(state.aiY), paddleWidth, paddleHeight); context.fillRect(Math.round(state.ballX), Math.round(state.ballY), ballSize, ballSize);
      context.font = '9px monospace'; context.fillText(String(state.playerScore), 36, 9); context.fillText(String(state.aiScore), 60, 9); context.fillStyle = 'rgba(0,0,0,.16)';
    ...
    draw();
    const canAnimate = () => active && inView && pageVisible && !reducedMotion && canvas.isConnected;
    ...
    return () => { if (frame !== undefined) cancelAnimationFrame(frame); canvas.removeEventListener('pointermove', pointerMove); canvas.removeEventListener('pointerdown', pointerDown); canvas.removeEventListener('keydown', keyDown); canvas.removeEventListener('keyup', keyUp); };
  }, [active, inView, pageVisible, reducedMotion]);

  return <canvas ref={canvasRef} className="pong-channel" width="104" height="88" tabIndex={0} role="application" aria-label="Playable Pong. Move your paddle with the mouse, Arrow keys, or W and S." />;
}
